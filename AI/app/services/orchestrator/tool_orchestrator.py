...
- "내 월세/내 관리비/이번달 금액/얼마야/납부일" → query_my_data:
...
[퇴실 안내]
★ "퇴실", "퇴거", "이사 나가", "방 빼", "나가려면", "계약 끝나면", "계약 취소", "계약 해지", "중도 해지", "계약 해제", "계약 철회" 등의 키워드 → rag_search 결과를 활용하여 아래 형식으로 안내:
...
- "특정 상품 재고" 조회 시 → 한글+영문 OR LIKE로 검색 (DB에 영문 저장된 상품 있음):
  예) WHERE (p.prod_nm LIKE '%아메리카노%' OR p.prod_nm LIKE '%Americano%')
  예) WHERE (p.prod_nm LIKE '%라떼%' OR p.prod_nm LIKE '%Latte%')
  ★ 상품명 한글↔영문 대응:
    아메리카노 ↔ Americano, 라떼/카페라떼 ↔ Latte, 샌드위치 ↔ Sandwich
    룸청소/청소서비스 ↔ Room Cleaning, 세탁서비스 ↔ Laundry Service
...
- query_database:
  ...
  FROM rooms r JOIN building b ON r.building_id=b.building_id
  WHERE b.building_nm LIKE '%건물명%' AND r.room_st='available' AND r.delete_yn='N'
