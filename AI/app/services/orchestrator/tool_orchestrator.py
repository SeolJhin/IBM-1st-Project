...
- "투어 예약/사전방문 신청/언제 가능/가능한 날":
  ...
  - ★ 방 목록 출력 규칙:
    - room_st='available'(입주 가능)인 방만 출력
    - 10개 이하면 전부 나열
    - 10개 초과면 대표 10개만 나열 후 "...외 N개 (더 많은 방은 '방찾기' 메뉴에서 확인하세요)" 추가
    - 마지막에 "예약 가능한 호수를 입력해 주세요." 유도
  - 사용자가 "아무거나", "추천해줘", "상관없어" 라고 하면 첫 번째 방을 자동 선택
  __BUTTONS__[{{"label":"방찾기","url":"/rooms","icon":"🔍"}}]

  [Step C] 가능 시간 안내 (방 확정 후) — ★ 날짜를 따로 묻지 말 것
  - 방이 확정되면 즉시 get_tour_available_slots 호출:
    room_id=<확정된 room_id>
  - 반환된 슬롯에서 available=true인 항목만 추려 날짜별로 묶어 안내:
    예시 출력:
    📅 1101호 투어 가능 시간 (가장 가까운 날짜 순)
    · 2026-03-24: 10:00, 14:00, 16:00 (available)
    · 2026-03-25: 10:00 (available) / 14:00 예약됨 / 16:00 (available)
  - ★ 투어 시간은 10:00 / 14:00 / 16:00 세 가지만 존재. 다른 시간은 없음.
  - 사용자가 10:00/14:00/16:00 외 시간을 입력하면 →
    "투어 예약 가능 시간은 10:00, 14:00, 16:00 중 하나입니다. 어떤 시간을 선택하시겠어요?"
    → 재선택 유도. 절대 같은 슬롯 목록 다시 출력 금지.

  [Step D] 시간 확정 후 예약 안내
  - 사용자가 10:00/14:00/16:00 중 하나를 선택하고 날짜도 확정되면:
    "✅ [날짜] [시간] 투어 예약을 진행하시겠어요? 아래 버튼을 눌러 예약해 주세요."
  ★ 반드시 버튼 추가:
...
- ★ "방 추천해줘", "방 찾아줘", "입주 가능한 방", "좋은 방 있어?" 등 방 추천 요청 시:
  [1단계] 로그인 여부 확인:
    - 비로그인(user_id 없음) → 바로 [3단계]로 이동
    - 로그인(user_id 있음) → [2단계] 계약 확인

  [2단계] 로그인 시 계약 만료 확인:
    query_my_data: SELECT c.contract_end, c.rent_price, r.room_type, r.room_size, r.pet_allowed_yn, b.building_id, b.building_nm FROM contract c JOIN rooms r ON c.room_id=r.room_id JOIN building b ON r.building_id=b.building_id WHERE c.user_id='{{user_id}}' AND c.contract_st='active' LIMIT 1
    - 계약 없음 → [3단계]로 이동
    - 계약 만료 30일 이내 (DATEDIFF(contract_end, CURDATE()) <= 30) → [계약만료 전 방추천] 플로우 실행 (아래 참조)
    - 계약 만료 30일 초과 → [3단계]로 이동

  [3단계] 일반 방 추천 (조건 질문):
    "원하시는 조건을 알려주세요. 예: 월세 범위, 반려동물 여부, 방 타입(원룸/투룸), 건물 선호도"
    조건이 하나라도 있으면 바로 query_database로 조회. 조건 없이 전체 조회 금지.
...
비로그인 → 위 [3단계] 일반 방 추천으로 진행. 로그인 안내 없이 조건 질문 후 바로 조회.
...
def _run(prompt: str, history: list[dict], user_id: str | None, provider: str) -> AiResponse:
    ...
    # ── 투어 진행 중 + 방 번호 입력 → room_id 미리 조회 후 슬롯 직접 반환 ────
    if _detect_ongoing_intent(history) == "tour":
        import re as _re_tour
        _room_no_match = _re_tour.search(r"(\d{2,4})\s*호?$", prompt.strip())
        if _room_no_match:
            _room_no = _room_no_match.group(1)
            # 이전 대화에서 건물명 추출
            _tour_building = ""
            for _msg in reversed(history or []):
                _c = _msg.get("content", "")
                if "Uniplace" in _c or "유니플레이스" in _c:
                    import re as _re_bn
                    _bn_match = _re_bn.search(r"(Uniplace\s*\w+|유니플레이스\s*\w*)", _c)
                    if _bn_match:
                        _tour_building = _bn_match.group(1)
                        break
            # room_id 조회
            _room_sql = f"SELECT r.room_id FROM rooms r JOIN building b ON r.building_id=b.building_id WHERE r.room_no='{_room_no}' AND r.delete_yn='N'"
            if _tour_building:
                _tour_building_clean = _tour_building.replace("'", "")
                _room_sql += f" AND b.building_nm LIKE '%{_tour_building_clean}%'"
            _room_sql += " LIMIT 1"
            _room_result = execute_tool(
                tool_name="query_database",
                tool_args={"sql": _room_sql, "description": f"투어 방번호 {_room_no} room_id 조회"},
                user_id=user_id,
            )
            _room_rows = _room_result.get("data") or [] if _room_result.get("success") else []
            if _room_rows:
                _resolved_room_id = _room_rows[0].get("room_id")
                if _resolved_room_id:
                    # 슬롯 직접 조회
                    _slot_result = execute_tool(
                        tool_name="get_tour_available_slots",
                        tool_args={"room_id": _resolved_room_id},
                        user_id=user_id,
                    )
                    _slot_docs = format_tool_result("get_tour_available_slots", _slot_result)
                    _slot_text = "\n".join(_slot_docs) if _slot_docs else "슬롯 조회 결과 없음"
                    # LLM에 결과만 전달하여 자연어 답변 생성
                    _tour_messages = _build_messages(prompt, history, user_id)
                    _tour_messages[-1]["content"] = (
                        f"사용자가 {_room_no}호 투어를 원합니다.\n\n"
                        f"[투어 가능 시간 조회 결과]\n{_slot_text}\n\n"
                        "위 결과를 날짜별로 정리해서 안내하세요. "
                        "사용자가 날짜와 시간을 선택하면 투어 예약을 진행할 수 있도록 안내하세요. "
                        "JSON, 코드블록, __METADATA__ 등 태그를 절대 출력하지 마세요. 순수 한국어 텍스트로만 답변하세요."
                    )
                    _tour_resp, _ = _call_with_fallback(
                        client, provider,
                        {"groq": settings.groq_model, "openai": settings.openai_model, "gemini": settings.gemini_model}.get(provider, settings.groq_model),
                        dict(temperature=0.1, max_tokens=1024, messages=_tour_messages),
                    )
                    if _tour_resp:
                        _tour_answer = (_tour_resp.choices[0].message.content or "").strip()
                        _tour_answer = _clean_function_call_text(_tour_answer)
                        # __METADATA__ 및 유사 태그 제거 (LLM 오타 포함)
                        _tour_answer = __import__("re").sub(r"__META\w*__\{.*?\}", "", _tour_answer).strip()
                        if _tour_answer:
                            return AiResponse(
                                answer=_tour_answer,
                                confidence=0.95,
                                metadata={
                                    "recommend_type": "tour_reserve",
                                    "room_id": _resolved_room_id,
                                },
                            )
                    logger.warning("[ToolOrchestrator] 투어 슬롯 LLM 답변 생성 실패")
...
"""
    ...
    resp3, used_model3 = _call_with_fallback(
        ...
        dict(temperature=0.1, max_tokens=4096, messages=final_messages),
    )
    ...
    if not answer or _is_tool_call_json(answer):
        ...
        resp_fb, _ = _call_with_fallback(
            ...
            dict(temperature=0.1, max_tokens=4096, messages=fallback_messages),
        )
        if resp_fb:
...
def _call_with_fallback(client, provider: str, start_model: str, kwargs: dict):
    ...
    # Gemini 키 목록 (429 시 다른 키로 재시도)
    _gemini_keys = _get_gemini_api_keys() if provider == "gemini" else []
    _tried_key_indices: set[int] = set()

    last_error = None
    for model in candidates:
        # Gemini: 현재 client의 키 포함 모든 키로 시도
        _key_attempts = list(range(len(_gemini_keys))) if _gemini_keys else [None]
        for _ki in _key_attempts:
            try:
                # 다른 키면 client 교체
                _cur_client = client
                if _ki is not None and _ki not in _tried_key_indices:
                    _tried_key_indices.add(_ki)
                    _cur_client = OpenAI(
                        api_key=_gemini_keys[_ki],
                        base_url=settings.gemini_base_url,
                        timeout=30.0,
                    )
                elif _ki is not None:
                    continue  # 이미 시도한 키 스킵

                logger.info("[ToolOrchestrator] LLM 호출 model=%s key_idx=%s", model, _ki)
                resp = _cur_client.chat.completions.create(model=model, **kwargs)
    ...
    logger.error("[ToolOrchestrator] 모든 모델/키 실패: %s", last_error)
...
# Gemini API 키 라운드로빈 카운터
_gemini_key_index = 0

def _get_gemini_api_keys() -> list[str]:
    """설정된 Gemini API 키 목록 반환 (비어있는 키 제외)."""
    keys = [
...
_ONGOING_INTENT_HINT = {
    ...
    "tour": (
        ...
        "현재 메시지는 방 번호 또는 시간 선택입니다. "
        "★ 사용자가 방 번호(예: '312호', '101')를 입력한 경우, room_id를 모르므로 반드시 아래 순서를 따르세요:\n"
        "  1단계: query_database로 room_id 조회 → "
        "SELECT r.room_id FROM rooms r JOIN building b ON r.building_id=b.building_id "
        "WHERE r.room_no='사용자가 말한 방번호' AND r.delete_yn='N' LIMIT 1\n"
        "  (이전 대화에서 건물명이 언급됐으면 WHERE 조건에 b.building_nm LIKE '%건물명%' 추가)\n"
        "  2단계: 조회된 room_id로 get_tour_available_slots(room_id=조회된ID) 호출\n"
        "★ room_id 없이 get_tour_available_slots를 호출하면 안 됩니다. 반드시 1단계를 먼저 수행하세요.\n"
        "투어 시간은 10:00/14:00/16:00 세 가지만 존재합니다. "
        "사용자가 10:00/14:00/16:00 외 다른 시간을 입력하면 '투어 예약 가능 시간은 10:00, 14:00, 16:00 중 하나입니다'라고 안내하고 재선택을 유도하세요. "
        "날짜를 먼저 묻지 말고 get_tour_available_slots 결과의 available=true 슬롯을 날짜별로 묶어 안내하세요."
    ),
    "complain": (
