...
GROQ_FALLBACK_MODELS = [
    ...
    "llama-3.1-8b-instant",                           # 500K TPD, 마지막 수단
]
...
def _run(prompt: str, history: list[dict], user_id: str | None, provider: str) -> AiResponse:
    ...
    # ★ 일부 모델이 tool_calls 대신 텍스트에 <function=...> 를 섞어 반환하는 경우 감지
    if "<function=" in raw_content or "<function_calls>" in raw_content:
        ...
        answer = (resp_clean.choices[0].message.content or "").strip() if resp_clean else                  "죄송합니다, 잠시 후 다시 시도해주세요."
