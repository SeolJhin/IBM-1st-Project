...
- "구매가능 품목/룸서비스 상품/건물 상품" → query_database:
  SELECT b.building_nm, pbs.prod_nm, pbs.prod_stock FROM product_building_stock pbs JOIN building b ON pbs.building_id=b.building_id WHERE b.delete_yn='N' ORDER BY b.building_nm, pbs.prod_nm
  건물명이 언급된 경우 WHERE b.building_nm LIKE '%건물명%' 추가
  → 결과를 건물별로 묶어서 출력. 재고 0이면 "⚠️품절" 표시.
- "특정 상품 재고" 조회 시 → 한글+영문 OR LIKE로 검색 (DB에 영문 저장된 상품 있음):
  예) WHERE (pbs.prod_nm LIKE '%아메리카노%' OR pbs.prod_nm LIKE '%Americano%')
  예) WHERE (pbs.prod_nm LIKE '%라떼%' OR pbs.prod_nm LIKE '%Latte%')
  ★ 상품명 한글↔영문 대응:
