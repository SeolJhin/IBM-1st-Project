...
[도구 선택 기준]
- "오늘 예약 몇 건?/예약 현황/예약 몇건" → 반드시 2개 쿼리를 순서대로 실행 (admin_stats 사용 금지):
  1) 사전방문 예약: query_database → SELECT COUNT(*) AS tour_count FROM room_reservation WHERE DATE(tour_start_at) = CURDATE()
  2) 공용공간 예약: query_database → SELECT COUNT(*) AS space_count FROM space_reservations WHERE DATE(sr_start_at) = CURDATE()
  → 결과: "📅 오늘 예약 현황\n• 사전방문 예약: N건\n• 공용공간 예약: N건\n• 합계: N건"
  ★ 반드시 두 가지를 모두 조회해서 함께 출력. 한쪽만 조회하거나 매번 다른 결과를 내지 말 것.
...
- "결제 문서/결제 내역 문서/건물별 결제/각 건물 결제/결제 리포트/결제 보고서/결제 내역 생성/결제 문서 만들어/결제 엑셀" →
  ...
  "결제 내역 리포트를 생성하겠습니다. 어느 건물, 몇 월 데이터로 만들까요? (예: 전체 빌딩 이번달 / {building_list_example} 3월)"
...
- "이번 달 결제 요약/결제 현황/결제 통계/결제 얼마야/이번달 결제 보여줘/결제 요약" →
  ...
  "어느 건물 결제 내역을 보여드릴까요? (전체 / {building_list})
...
- "미납자/미납 현황/미납 목록/연체 현황/연체자/미납 요약" →
  ★ 반드시 아래 두 가지를 먼저 한번에 질문 (절대 tool call 먼저 하지 말 것):
  "어느 건물 미납 현황을 보여드릴까요? (전체 / {building_list})
  조회 기간을 알려주세요. (전체 / 이번 달)"
  ★ 사용자가 건물과 기간을 답한 후에만 admin_stats 실행:
    - 전체+전체: admin_stats(metric=unpaid_charges, period=all)
    - 전체+이번달: admin_stats(metric=unpaid_charges, period=month)
    - 특정건물+전체: admin_stats(metric=unpaid_charges, period=all, building_id=건물ID)
    - 특정건물+이번달: admin_stats(metric=unpaid_charges, period=month, building_id=건물ID)
  ★ building_id에는 반드시 숫자(정수)만 사용. 건물명을 넣지 말 것.
  ★ today/week 절대 금지. period는 all 또는 month만 사용.
  ★★★ 결과는 반드시 아래처럼 마크다운 테이블(| 구분자)로 출력. 불릿(•) 리스트 형식 절대 금지 ★★★
  unpaid_months가 유효한 날짜(YYYY-MM)가 아닌 값(0, 1 등)이면 미납월에 "-" 표시하고 unpaid_count로 "N개월"만 표시.
  출력 예시(반드시 이 형식 준수):
  "⚠️ 미납 현황 — 총 N명 / 합계 N원

  | 이름 | 건물 | 호수 | 미납월 | 미납개월 | 미납금액 |
  |---|---|---|---|---|---|
  | 홍길동 | Uniplace A | 101호 | 2026-01, 2026-02 | 2개월 | 1,000,000원 |
  | 김철수 | Uniplace B | 205호 | - | 3개월 | 1,500,000원 |"
  답변 끝에 반드시: __BUTTONS__[{{"label":"월세 청구 관리","url":"/admin/pay/billings","icon":"🏠"}}]

- "월세 현황/월세 조회/월세 납부 현황/월별 월세/전체 월세" →
  ★ 반드시 아래 두 가지를 먼저 한번에 질문 (절대 tool call 먼저 하지 말 것):
  "어느 건물 월세 현황을 보여드릴까요? (전체 / {building_list})
  조회 기간을 알려주세요. (전체 / 특정 월 입력 예: 2026-03)"
  ★ 사용자가 답한 후 query_database 실행:
  - 전체 건물 + 전체: SELECT mc.billing_dt AS billing_month, COUNT(*) AS total_cnt, SUM(CASE WHEN mc.charge_st='paid' THEN 1 ELSE 0 END) AS paid_cnt, SUM(CASE WHEN mc.charge_st IN ('unpaid','overdue') THEN 1 ELSE 0 END) AS unpaid_cnt, IFNULL(SUM(mc.price),0) AS total_price, IFNULL(SUM(CASE WHEN mc.charge_st='paid' THEN mc.price ELSE 0 END),0) AS paid_price FROM monthly_charge mc WHERE mc.billing_dt IS NOT NULL AND mc.billing_dt != '' AND mc.billing_dt >= DATE_FORMAT(DATE_SUB(NOW(), INTERVAL 5 MONTH), '%Y-%m') GROUP BY mc.billing_dt ORDER BY mc.billing_dt DESC
  - 특정 건물: JOIN contract c ON mc.contract_id=c.contract_id JOIN rooms r ON c.room_id=r.room_id JOIN building b ON r.building_id=b.building_id AND b.building_nm LIKE '%건물명%' 추가
  - 특정 월 지정 시: WHERE mc.billing_dt='YYYY-MM' (6개월 제한 없이 해당 월만 조회)
  → 결과를 표 형식으로 출력:
  "📊 월세 납부 현황
  | 청구월 | 총건수 | 납부완료 | 미납 | 총청구액 | 납부액 |"
  답변 끝: __BUTTONS__[{{"label":"월세 청구 관리","url":"/admin/pay/billings","icon":"🏠"}}]

- "N개월 이내 만료/만료 임박/곧 끝나는 계약/만료 예정" → 건물 되묻기 절대 금지. 즉시 query_database 실행:
  ★ N은 사용자가 말한 숫자(예: "3개월 이내" → 3). 언급 없으면 기본 3개월.
  SQL: SELECT u.user_nm, b.building_nm, r.room_no, c.contract_start, c.contract_end,
       ...
       WHERE c.contract_st='active'
         AND c.contract_end BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL N MONTH)
       ORDER BY c.contract_end ASC LIMIT 50
  결과: 반드시 건물별로 마크다운 표를 따로 출력:
  "📋 N개월 이내 만료 예정 계약 (총 M건)

  **Uniplace A**
  | 호수 | 이름 | 계약시작 | 계약종료 | 남은일 |
  |---|---|---|---|---|
  | 101호 | jinung1 | 2025-04-19 | 2026-04-19 | 23일 |

  **Uniplace B**
  | 호수 | 이름 | 계약시작 | 계약종료 | 남은일 |
  |---|---|---|---|---|"
  ★ 건물별로 소제목 + 표를 각각 생성. 절대 bullet(•) 형식 사용 금지.
  답변 끝: __BUTTONS__[{{"label":"계약 관리","url":"/admin/contracts","icon":"📝"}}]
...
- 반드시 LIMIT 50 포함
- 빌딩명은 영문: {building_list} (DB의 실제 building_nm 사용)
...
[커뮤니티 게시글 조회]
★ board 실제 컬럼: board_id, board_title, board_ctnt, user_id, created_at, updated_at, read_count, code, anonymity, importance, file_ck, reply_ck
★ like_count 컬럼 없음 — board_likes 테이블 COUNT JOIN 필요
★ delete_yn / is_blind 컬럼 없음 — 해당 조건 절대 사용 금지

- "제목에 [키워드]/제목이 [키워드]/[키워드] 포함된 글/[키워드] 검색" → query_database:
  SELECT b.board_id, b.board_title, b.user_id, b.read_count, b.created_at
  FROM board b
  WHERE b.board_title LIKE '%[키워드]%'
  ORDER BY b.created_at DESC LIMIT 20

- "내용에 [키워드]/본문 검색" → query_database:
  SELECT b.board_id, b.board_title, b.user_id, b.read_count, b.created_at
  FROM board b
...
- "좋아요 많은 글/인기 글/좋아요 순" → query_database:
  ...
  ORDER BY like_cnt DESC LIMIT 10
...
[민원(Complain) 현황]
- "민원 처리 현황/민원 현황/민원 목록" → 아래 2개 SQL을 순서대로 실행:
  1) 상태별 요약: SELECT comp_st, COUNT(*) AS cnt FROM complain WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) GROUP BY comp_st
  2) 7일 이상 미처리 민원: SELECT comp_id, comp_title, comp_st, importance, created_at FROM complain WHERE comp_st IN ('pending','received','in_progress') AND created_at < DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY created_at ASC LIMIT 10
  → 결과를 아래 형식으로 출력:
  "📋 최근 7일 민원 현황 요약
  • 접수대기: N건 / 접수완료: N건 / 처리중: N건 / 해결됨: N건

  ⚠️ 장기 미처리 민원 (7일 이상)
  • #ID 제목 | 상태 | 중요도 | 접수일"
  → 장기 미처리 없으면 "장기 미처리 민원 없음 ✅" 출력
- "미처리만" → WHERE comp_st IN ('pending','received','in_progress') ORDER BY created_at ASC LIMIT 20
- "중요도 높은 민원" → WHERE importance='high' ORDER BY created_at DESC LIMIT 10
...
[답변 원칙]
- ★ 대화 흐름 유지: 사용자에게 건물/기간 등을 되물은 직후, 사용자의 다음 메시지는 그 질문에 대한 답변이다. 절대 새로운 질문으로 해석하지 말고 해당 흐름을 이어서 처리할 것.
- 데이터는 표 형식(마크다운)으로 정리
- 이상 징후 발견 시 ⚠️ 표시
- ★ 웹 검색(web_search) 결과를 답변에 사용할 때 반드시 출처를 포함하세요:
  형식: "출처: [사이트명](URL)" 또는 "📎 출처: URL"
  출처 없이 웹 검색 결과를 답변하면 안 됩니다. 신뢰성을 위해 반드시 어디서 가져온 정보인지 명시하세요.
...
_STATS_SQL = {
    ...
    ("unpaid_charges", "all"):
        """SELECT u.user_nm, c.user_id, b.building_nm, r.room_no,
                  COUNT(mc.charge_id) AS unpaid_count,
           ...
           JOIN building b ON r.building_id = b.building_id
           JOIN users u ON c.user_id = u.user_id
           WHERE mc.charge_st IN ('unpaid', 'overdue')
             AND mc.billing_dt IS NOT NULL AND mc.billing_dt != ''
...
def _handle_admin_stats(args: dict, admin_user_id: str, tool_call_id: str) -> dict:
    ...
    if building_id:
        # building_id가 숫자가 아닌 경우 (LLM이 건물명을 전달한 경우) → building_nm LIKE 조건으로 변환
        try:
            bid = int(building_id)
            bid_condition = f"b.building_id = {bid}"
        except (ValueError, TypeError):
            bid_condition = f"b.building_nm LIKE '%{str(building_id).replace(chr(39), '')}%'"
        # LIMIT/ORDER BY 앞에 조건 삽입
        import re as _re_sql
        limit_match = _re_sql.search(r'(GROUP BY|ORDER BY|LIMIT)\s', base_sql, _re_sql.IGNORECASE)
        if limit_match:
            insert_pos = limit_match.start()
            base_sql = base_sql[:insert_pos] + f" AND {bid_condition} " + base_sql[insert_pos:]
        elif "WHERE" in base_sql.upper():
            base_sql = base_sql.rstrip() + f" AND {bid_condition}"
        else:
            base_sql = base_sql.rstrip() + f" WHERE {bid_condition}"
...
""".strip()
    ...
    if overdue:
        ...
        for row in overdue[:10]:
            ...
            lines.append(
                ...
                f"| {row.get('user_nm','-')} "
                f"| 매월 {row.get('payment_day','-')}일 | {days}일 |"
            )
        if len(overdue) > 10:
    ...
    if terminated:
        ...
        lines.append("| 건물 | 호수 | 이름 | 계약기간 | 해지일 | 비고 |")
        lines.append("|---|---|---|---|---|---|")
        for row in terminated[:8]:
            ...
            lines.append(
                f"| {row.get('building_nm','-')} | {row.get('room_no','-')}호 "
                f"| {row.get('user_nm','-')} "
                f"| {days}일 | {row.get('created_at','-')} | {short_flag} |"
            )
        if len(terminated) > 8:
    ...
    if surge_action.get("action") in ("ALERT", "MONITOR") and new_contracts:
        ...
        lines.append("| 건물 | 호수 | 이름 | 상태 | 신청일 |")
        lines.append("|---|---|---|---|---|")
        for row in new_contracts[:8]:
            lines.append(
                f"| {row.get('building_nm','-')} | {row.get('room_no','-')}호 "
                f"| {row.get('user_nm','-')} "
                f"| {row.get('contract_st','-')} "
...
def _run_admin(prompt: str, history: list[dict], admin_id: str, provider: str, slots: dict = None, req: AiRequest = None) -> AiResponse:
    ...
    if _is_billing_report:
        ...
        try:
            ...
            # 빌딩명 파싱 — DB에서 동적으로 건물 목록 조회
            _bld_map = {}
            for _b in get_buildings():
                _nm = _b.get("building_nm", "")
                if not _nm:
                    continue
                _bld_map[_nm] = _nm  # 영문명 직접 입력
                # 알파벳 suffix 기반 한글 alias 자동 생성 (예: "Uniplace A" → "유니플레이스 A", "A동")
                import re as _re
                _suffix_m = _re.search(r'\s+([A-Za-z0-9]+)$', _nm)
                if _suffix_m:
                    _sfx = _suffix_m.group(1).upper()
                    _bld_map[f"유니플레이스 {_sfx}"] = _nm
                    _bld_map[f"유니플{_sfx}"] = _nm
                    _bld_map[f"{_sfx}동"] = _nm
            _target_bld = None
    ...
    if _is_price_query:
        ...
        for _b in get_buildings():
            ...
            if not _nm:
                continue
            _building_map[_nm] = _nm
            import re as _re
            _suffix_m = _re.search(r'\s+([A-Za-z0-9]+)$', _nm)
            if _suffix_m:
                _sfx = _suffix_m.group(1).upper()
                _building_map[f"유니플레이스 {_sfx}"] = _nm
                _building_map[f"유니플{_sfx}"] = _nm
                _building_map[_sfx] = _nm   # "A", "B" 단독 입력도 허용
        _building_nm = None
    ...
    # ── 건물 목록 동적 조회 ────────────────────────────────────────────────────
    _buildings = get_buildings()
    _building_names = [b["building_nm"] for b in _buildings if b.get("building_nm")]
    _building_list = " / ".join(_building_names) if _building_names else "Uniplace A / Uniplace B / Uniplace C"
    _building_list_example = _building_names[0] if _building_names else "Uniplace A"
    ...
    system_prompt = _ADMIN_SYSTEM_PROMPT_TEMPLATE.format(
        ...
        building_list_example=_building_list_example,
    )
    ...
    # 히스토리 맥락 강화 — 짧은 후속 답변도 이전 대화와 연결
    context_instruction = ""
    if history and len(history) >= 2:
        last_assistant = ""
        last_user_prompt = ""
        for h in reversed(history):
            if h.get("role") == "assistant" and not last_assistant:
                last_assistant = h.get("content", "")
            elif h.get("role") == "user" and not last_user_prompt:
                last_user_prompt = h.get("content", "")
            if last_assistant and last_user_prompt:
                break
        if last_assistant:
            context_instruction = (
                "\n\n⚠️ [대화 맥락 필수 참조]\n"
                f"직전 assistant 답변: \"{last_assistant[:300]}\"\n"
                f"직전 user 질문: \"{last_user_prompt[:100]}\"\n"
                f"현재 user 입력: \"{prompt}\"\n"
                "★ 현재 입력이 짧더라도(예: '3개월', '유니플레이스A 2월') 반드시 직전 대화 맥락과 연결하여 처리하세요. "
                "새로운 주제로 해석하지 마세요. 직전에 질문한 내용의 후속 답변입니다."
            )

    messages = [{"role": "system", "content": system_prompt + context_instruction}]
    for h in (history or []):
    ...
    if not (choice1.finish_reason == "tool_calls" and choice1.message.tool_calls):
        ...
        _needs_tool = bool(re.search(
            ...
            r"|만료|공실|방.*있|무슨.*방|어떤.*방|방.*목록|호실|호수"
            r"|게시글|게시판|커뮤니티|인기.*글|좋아요|댓글|조회수|공지사항|최신.*글|최근.*글",
            prompt
        ))
    ...
    for _round in range(MAX_ROUNDS):
        ...
        for tc in tool_calls:
            ...
            if tool_name == "web_search":
                ...
                # 검색 결과가 너무 길면 상위 3개만 사용 (입력 토큰 절약 → 출력 잘림 방지)
                if len(formatted_text) > 2000:
                    lines = formatted_text.split("\n\n")
                    formatted_text = "\n\n".join(lines[:4])
                formatted_text += "\n\n★ 위 검색 결과의 출처 URL을 답변에 반드시 포함하세요. 형식: 📎 출처: URL"
            ...
            # ── admin_stats ──────────────────────────────────────────────────
                ...
                try:
                    ...
                    logger.error("[AdminOrchestrator] admin_stats 실행 오류: %s", e)
...
def _format_admin_stats_answer(args: dict, stats_data: dict) -> str | None:
    ...
    if metric == "unpaid_charges":
        ...
        lines.append("| 이름 | 건물 | 호수 | 미납개월 | 미납금액 |")
        lines.append("|---|---|---|---|---|")
        for r in rows:
            name     = r.get("user_nm") or "알 수 없음"
            building = r.get("building_nm") or ""
            room     = r.get("room_no") or ""
            count    = _unpaid_count(r)
            amount   = _amount(r)
            lines.append(f"| {name} | {building} | {room}호 | {count}개월 | {amount:,}원 |")
    ...
    if metric == "occupancy":
        ...
        lines.append("| 건물 | 총 방수 | 계약중 | 공실 | 점유율 |")
        lines.append("|---|---|---|---|---|")
        for r in rows:
            ...
            contracted = r.get("contracted") or r.get("occupied_rooms") or r.get("occupied") or 0
            available  = r.get("available") or 0
            lines.append(f"| {building} | {total}실 | {contracted}실 | {available}실 | {occ}% |")
        return "\n".join(lines)
