...
/*
 * {room.files?.map(f => (
  	<img key={f.fileId} src={f.viewUrl} alt={f.originFilename} />))}
  	
 * <img src={room.thumbnailUrl} alt="thumbnail" />
 * */
...
public class RoomServiceImpl implements RoomService {
    ...
    public PageResponse<RoomSummaryResponse> search(RoomSearchRequest request, Pageable pageable) {
        ...
        // 여기서 “첫 번째 이미지”를 썸네일로 계산해서 내려줌
        Page<RoomSummaryResponse> mapped = page.map(room -> {
            List<FileResponse> files = fileService.getActiveFiles(FileRefType.ROOM.dbValue(), room.getRoomId());
