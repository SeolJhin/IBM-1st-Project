...
CREATE TABLE `ai_room_recommendation` (
  `rec_id`         int          NOT NULL AUTO_INCREMENT,
  `room_id`        int          NOT NULL,
  `rank_no`        int          NOT NULL          COMMENT '추천 순위 (1·2·3)',
  `ai_reason`      varchar(500) DEFAULT NULL      COMMENT 'AI가 생성한 추천 이유',
  
  `avg_rating`     decimal(3,2) DEFAULT NULL      COMMENT '평균 평점',
  `review_count`   int          DEFAULT NULL      COMMENT '리뷰 수',
  `contract_count` int          DEFAULT NULL      COMMENT '활성 계약 수',
  `generated_at`   datetime     NOT NULL
                   DEFAULT CURRENT_TIMESTAMP      COMMENT '추천 생성 시각',
  PRIMARY KEY (`rec_id`),
  KEY `ix_arr_generated` (`generated_at`),
  KEY `ix_arr_room`      (`room_id`),
  CONSTRAINT `fk_arr_room`
    FOREIGN KEY (`room_id`) REFERENCES `rooms` (`room_id`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_0900_ai_ci
  COMMENT='AI 홈 방 추천 Top3 이력';
...
created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, 
comp_st ENUM('received','in_progress','resolved') NOT NULL DEFAULT 'in_progress', importance ENUM('high','medium','low') NULL COMMENT 'AI 분류 중요도', 
ai_reason VARCHAR(500) NULL COMMENT 'AI 분류 근거', 
code VARCHAR(20) NOT NULL, 
file_ck VARCHAR(1) NOT NULL DEFAULT 'N', 
reply_ck VARCHAR(1) NOT NULL DEFAULT 'N', 
reply_ctnt VARCHAR(2000) NULL COMMENT '관리자 답변 내용', 
KEY ix_complain_user (user_id), 
KEY ix_complain_code (code), 
CONSTRAINT fk_complain_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE, CONSTRAINT fk_complain_code FOREIGN KEY (code) REFERENCES common_code(code) ) ENGINE=InnoDB;
...
CREATE TABLE notification (
  ...
  is_read_at      DATETIME,
  target_id       INT,
  target          ENUM('board', 'reply','notice','tour','space','review','payment','order','contract','support','security'),
...
CREATE TABLE inspection (
  ...
  space_type      ENUM('room','building','common_space') NOT NULL  COMMENT '점검 대상 공간 종류',
  space_id        INT            NOT NULL                          COMMENT '공간 PK',
  inspector_id    VARCHAR(50)    NOT NULL                          COMMENT '점검자 (관리자) user_id',
  before_file_id  INT            NULL                              COMMENT '이전 점검 이미지 file_id (첫 점검은 NULL)',
  after_file_id   INT            NOT NULL                          COMMENT '금번 점검 이미지 file_id',
  diff_file_id    INT            NULL                              COMMENT 'AI 생성 차이 이미지 file_id',
  change_percent  DECIMAL(5,2)   NULL                              COMMENT 'AI 계산 변화율(%)',
  inspection_st   ENUM('completed','issue_detected','no_change') NOT NULL DEFAULT 'completed' COMMENT '점검 결과 상태',
  inspection_memo VARCHAR(1000)  NULL                              COMMENT '점검 메모',
  created_at      DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at      DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,


  KEY ix_inspection_space     (space_type, space_id),
  KEY ix_inspection_inspector (inspector_id),
  KEY ix_inspection_created   (created_at),


  CONSTRAINT fk_inspection_inspector   FOREIGN KEY (inspector_id)   REFERENCES users(user_id),
  CONSTRAINT fk_inspection_before_file FOREIGN KEY (before_file_id) REFERENCES files(file_id),
  CONSTRAINT fk_inspection_after_file  FOREIGN KEY (after_file_id)  REFERENCES files(file_id),
  CONSTRAINT fk_inspection_diff_file   FOREIGN KEY (diff_file_id)   REFERENCES files(file_id)
) ENGINE=InnoDB COMMENT='AI 건물 점검 기록';


CREATE TABLE maintenance_ticket (
  ticket_id     INT           AUTO_INCREMENT PRIMARY KEY,
  inspection_id INT           NOT NULL                    COMMENT '생성 원인 점검 ID',
  space_type    ENUM('room','building','common_space') NOT NULL COMMENT '점검 대상 공간 종류',
  space_id      INT           NOT NULL                    COMMENT '공간 PK',
  issue_type    VARCHAR(50)   NOT NULL                    COMMENT '문제 유형 (wall_crack 등)',
  severity      ENUM('low','medium','high','critical') NOT NULL DEFAULT 'medium' COMMENT 'AI 판단 심각도',
  description   VARCHAR(1000) NULL                        COMMENT 'AI 설명',
  ticket_st     ENUM('open','in_progress','resolved','closed') NOT NULL DEFAULT 'open' COMMENT '처리 상태',
  created_at    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at    DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,


  KEY ix_ticket_inspection (inspection_id),
  KEY ix_ticket_space      (space_type, space_id),
  KEY ix_ticket_st         (ticket_st),


  CONSTRAINT fk_ticket_inspection FOREIGN KEY (inspection_id) REFERENCES inspection(inspection_id)
) ENGINE=InnoDB COMMENT='AI 점검 유지보수 티켓';
